...
/**
 * 获取黑名单列表
 * 详细细节 https://developers.weixin.qq.com/doc/offiaccount/User_Management/Manage_blacklist.html
 * Examples:
 * ```
 * api.getBlackList(beginOpenId);
 * ```
 ...
 * ```
 */
exports.getBlackList = async function (beginOpenId) {
  const { accessToken } = await this.ensureAccessToken();
  // https://api.weixin.qq.com/cgi-bin/tags/members/getblacklist?access_token=ACCESS_TOKEN
  var url = this.prefix + 'tags/members/getblacklist?access_token=' + accessToken;
  var data = {
 ...
 * ```
 */
exports.batchBlack = async function (openIds) {
  const { accessToken } = await this.ensureAccessToken();
  // https://api.weixin.qq.com/cgi-bin/tags/members/batchblacklist?access_token=ACCESS_TOKEN
  var url = this.prefix + 'tags/members/batchblacklist?access_token=' + accessToken;
  var data = {
    openid_list: openIds,
  };
  return this.request(url, postJSON(data));
 ...
 * ```
 */
exports.batchUnBlack = async function (openIds) {
  const { accessToken } = await this.ensureAccessToken();
  // https://api.weixin.qq.com/cgi-bin/tags/members/batchunblacklist?access_token=ACCESS_TOKEN
  var url = this.prefix + 'tags/members/batchunblacklist?access_token=' + accessToken;
  var data = {
    openid_list: openIds,
  };
  return this.request(url, postJSON(data));
