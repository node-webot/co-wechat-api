...
exports.registerTicketHandle = function (getTicketToken, saveTicketToken) {
  ...
  this.getTicketToken = getTicketToken || function* (type) {
    type = type || 'jsapi';
    return this.ticketStore[type];
  };

  this.saveTicketToken = saveTicketToken || function* (type, ticketToken) {
    // 向下兼容
...
exports.getTicket = function* (type) {
  var token = yield this.ensureAccessToken();
  type = type || 'jsapi';

  var url = this.prefix + 'ticket/getticket?access_token=' + token.accessToken + '&type=' + type;
  var data = yield this.request(url, {dataType: 'json'});

  // 过期时间，因网络延迟等，将实际过期时间提前10秒，以防止临界点
  var expireTime = Date().now() + (data.expires_in - 10) * 1000;
  var ticket = new Ticket(data.ticket, expireTime);
  yield this.saveTicketToken(type, ticket);
  return ticket;
};
...
exports.ensureTicket = function* (type) {
  var cache = yield this.getTicketToken(type);
  ...
  if (!ticket || !ticket.isValid()) {
    // 从微信端获取ticket
    ticket = yield this.getTicket(type);
  }
  return ticket;
};
...
exports.getJsConfig = function* (param) {
  var ticket = yield this.ensureTicket('jsapi');
 ...
 * @param {Object} param 参数
 */
exports.getCardExt = function* (param) {
  var apiTicket = yield this.ensureTicket('wx_card');
  var timestamp = createTimestamp();
 ...
 * - `ticket`, 获取的ticket
 */
exports.getLatestTicket = function* () {
  return yield this.ensureTicket('jsapi');
};
