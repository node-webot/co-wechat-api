...
/**
 * 邀请绑定客服帐号
 * 详细请看：https://mp.weixin.qq.com/wiki?t=resource/res_main&id=mp1458044813&token=&lang=zh_CN * Examples:
 * ```
 * var result = yield* api.inviteworker('test@test', 'invite_wx');
 * ```
 * Result:
...
exports.inviteworker = function* (account, wx) {
  var token = yield* this.ensureAccessToken();
  // https://api.weixin.qq.com/customservice/kfaccount/inviteworker?access_token=ACCESS_TOKEN  
  var url = this.prefix + 'customservice/kfaccount/inviteworker?access_token=' + token.accessToken;
  var data = {
    "kf_account": account,
    "invite_wx": wx,    
  };
